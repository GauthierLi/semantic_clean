"""Convert subtype `.npy` datasets into subtype JSON, binary protobuf, or manual review JSON.

`--manual` outputs the same review-oriented JSON structure as `sem_clean --clean --output`,
with every case and category defaulting to `review` for manual cleaning workflows.
"""
...
def proto_message_to_json_dict(message: clean_subtype_pb2.ImageData) -> dict:
    """Convert protobuf message to JSON-compatible dict"""
    json_str = MessageToJson(message, preserving_proto_field_name=True)
    return json.loads(json_str)


def proto_message_to_manual_review_result(message: clean_subtype_pb2.ImageData) -> dict:
    """Convert protobuf message to sem_clean review-style JSON result"""
    category_results = []
    for category in message.category:
        category_results.append({
            'category': clean_subtype_pb2.ImageCategory.Name(category),
            'decision': 'review',
            'score': 0.0,
            'metrics': {
                'knn_consistency': 0.0,
                'nearest_distance_normalized': 0.0,
                'class_distance_normalized': 0.0
            },
            'error': None
        })

    return {
        'image_id': str(message.id),
        'image_path': message.image_path,
        'decision': 'review',
        'score': 0.0,
        'categories': category_results,
        'total_categories': len(message.category),
        'validated_categories': len(category_results),
        'error': None
    }


def process_input_to_json(input_path: str, output_path: str, img_root: str, manual: bool = False) -> None:
    ...
    try:
        ...
        if manual:
            json_list = [proto_message_to_manual_review_result(msg) for msg in proto_messages]
        else:
            json_list = [proto_message_to_json_dict(msg) for msg in proto_messages]
        
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(json_list, f, indent=2, ensure_ascii=False)
            
        output_mode = 'manual review JSON' if manual else 'JSON'
        print(f"Successfully converted {len(proto_messages)} items to {output_path} ({output_mode})")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--manual",
        action="store_true",
        help="Output sem_clean-compatible review JSON for manual cleaning"
    )

    args = parser.parse_args()

    if args.binary and args.manual:
        parser.error("--manual cannot be used together with --binary")
    
    if args.binary:
        process_input_to_binary(args.input_path, args.output_file, args.img_root)
    else:
        process_input_to_json(args.input_path, args.output_file, args.img_root, manual=args.manual)
