...
2. **Generate Manual Review JSON**
```bash
# Output the same structure as `sem_clean --clean --output`, with every case defaulting to `review`
python scripts/dataset_transforms/subtype.py data/subtype_251204_mideast-train.npy manual_review.json --img-root ~/data/subtype/train_images_dir --manual
```
...
### Subtype Dataset Transform
```bash
python scripts/dataset_transforms/subtype.py <input_path> <output_file> --img-root <image_root>
python scripts/dataset_transforms/subtype.py <input_path> <output_file> --img-root <image_root> --manual
```

- `input_path`: `.npy` file, directory of `.npy` files, or `.txt` file containing `.npy` paths
- `output_file`: Output `.json` or `.bin` file path
- `--img-root`: Root directory used to build `image_path`
- `--binary`: Write protobuf binary output instead of standard metadata JSON
- `--manual`: Write sem_clean-compatible manual review JSON; cannot be combined with `--binary`
...
[
  {
    ...
    "categories": [
      ...
      },
      {
        ...
        "score": 0.0,
        "metrics": {
          "knn_consistency": 0.61,
          "nearest_distance_normalized": 0.44,
          "class_distance_normalized": 0.38
        },
        "error": null
      }
    ],
    "total_categories": 2,
    "validated_categories": 2,
    "error": null
  }
]
```
...
- `decision`: Final image-level decision (`accept` / `reject` / `review`)
- `score`: Average score across validated categories
- `categories`: Per-category validation results
- `categories[].category`: Category name
- `categories[].decision`: Category-level decision
- `categories[].score`: Category-level confidence score
- `categories[].metrics`: Detailed algorithm metrics for the category
- `total_categories`: Number of categories carried by the sample
- `validated_categories`: Number of categories actually validated
- `error`: Error information (if any processing exception)

When using `python scripts/dataset_transforms/subtype.py ... --manual`, the generated JSON follows this same schema, but every image and category starts with `decision = "review"` and `score = 0.0` so it can be used directly for manual cleaning.
