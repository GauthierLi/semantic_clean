...
def correct_sample_category(sample: Dict, source_category: str, target_category: str) -> bool:
    """修正样本类别，并将修正后的类别状态设为accept"""
    ...
    for index, cat_info in enumerate(categories):
        if not isinstance(cat_info, dict):
            continue
        if cat_info.get('category') == source_category and source_index is None:
            source_index = index
        if cat_info.get('category') == target_category and target_index is None:
            target_index = index
    ...
    if target_index is not None and target_index != source_index:
        categories[target_index]['decision'] = 'accept'
        categories.pop(source_index)
    else:
        categories[source_index]['category'] = target_category
        categories[source_index]['decision'] = 'accept'

    return True

def refresh_current_samples(samples: List[Dict[str, Any]]) -> None:
    """同步内存中的样本缓存"""
    current_data['all_samples'] = samples
    current_data['review_samples'] = [sample for sample in samples if sample.get('decision') == 'review']
...
def update_decisions():
    ...
    try:
        ...
        refresh_current_samples(copy_data)
...
@app.route('/api/correct_category', methods=['POST'])
def correct_category():
    """修正选中样本的类别，并将修正后的类别状态设为accept"""
    try:
        data = request.get_json()
        if not data:
            return jsonify({
        ...
        if not image_ids:
            return jsonify({
                'success': False,
                'error': 'Missing image_ids parameter'
            }), 400

        if not source_category or source_category == 'all':
            return jsonify({
                'success': False,
                'error': 'Please select a specific source category'
            }), 400

        if not target_category or target_category == 'all':
            return jsonify({
                'success': False,
                'error': 'Please select a valid target category'
            }), 400
        ...
        for sample in copy_data:
            if sample.get('image_id') not in image_id_set:
                continue

            if correct_sample_category(sample, source_category, target_category):
                update_overall_decision(sample)
                corrected_count += 1
        ...
        refresh_current_samples(copy_data)
        logger.info(
            f"Corrected category from {source_category} to {target_category} for {corrected_count} samples"
        )
        ...
        logger.error(f"Error in correct_category: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
